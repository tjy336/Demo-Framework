...
3.架构支持
   电脑：仅支持 arm64 架构（如 iPhone、iPad 等设备）。
...
兼容性说明
框架通过 XCFramework 格式同时支持电脑和模拟器，无需为不同环境单独配置。
如果项目需兼容较旧的 iOS 版本（如 iOS 10 及以下），需确认框架的最低支持版本（文档中未明确提及，建议检查 Deployment Target）。
如果使用 Swift Package Manager 集成，需确保项目使用 Xcode 11 及以上版本。
